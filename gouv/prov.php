...
<script>
  ...
  var vlaRes=<?php
include 'http://www.vlaanderenkiest.be/2012/provincie/'.$vlaId.'/entiteitUitslag.json';
?>;
  var vlaLists=<?php
include 'http://www.vlaanderenkiest.be/2012/provincie/'.$vlaId.'/entiteitLijsten.json';
?>;
  data = {"sieges": {}, progrInit : {}};
for (var part in vlaRes.P[vlaId].us) {
  //alert(vlaLists.G[part].nm);
  //alert(vlaRes.G[vlaId].us[part].zs);
  if (vlaRes.P[vlaId].us[part].zs>0) {
    data.sieges[vlaLists.P[part].nm] = vlaRes.P[vlaId].us[part].zs;
    data.progrInit[vlaLists.P[part].nm] = { "require":[], "reject":[] };
    color[vlaLists.P[part].nm] = vlaLists.P[part].kl;
  }
}
  <?php 
  } else if ($walId) { ?>
  ...
  data = {"sieges": {
<?php
 $html = file_get_contents('http://elections2012.wallonie.be/results/fr/pro/results/results_tab_'.$walId.'.html');
 $DOM = new DOMDocument;
 $DOM->loadHTML($html);
...
<body>
  ...
  <h3>Toi aussi, forme ton Conseil Provincial</h3>
  <?php
  if ($walId) {
